...
def remove_mean_with_mask_y(x, node_mask, y):
    masked_max_abs_value = (x * (1 - node_mask)).abs().sum().item()
    assert masked_max_abs_value < 1e-5, f'Error {masked_max_abs_value} too high'
    # remove the current atom position
    N = node_mask.sum(1, keepdims=True)
    # a = N[1] avoid divide by 0
    N = torch.where(N > 0, N, torch.tensor(1, dtype=N.dtype,device=N.device))
...
def assert_mean_zero_with_mask_y(x, node_mask, y, eps=1e-10):
    assert_correctly_masked(x, node_mask)
    largest_value = x.abs().max().item()
    error = (torch.sum(x, dim=1, keepdim=True)).abs().max().item()
...
def sample_center_gravity_zero_gaussian_with_mask_y(size, device, node_mask, single_node_mask, y):
    # this y is the condition graph
    assert len(size) == 3
    x = torch.randn(size, device=device)

    x_masked = x * single_node_mask

    # This projection only works because Gaussian is rotation invariant around
    # zero and samples are independent!
    x_projected = remove_mean_with_mask_y(y, node_mask, x_masked)
