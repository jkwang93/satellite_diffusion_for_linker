...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True):
    ...
    # todo add a fake max=30 molecules, in order to padding

    molecules[0]['charges'] = torch.nn.ConstantPad1d((0, 30 - molecules[0]['charges'].shape[0]), 0)(
        molecules[0]['charges'])
    molecules[0]['positions'] = torch.nn.ConstantPad2d((0, 0, 0, 30 - molecules[0]['positions'].shape[0]), 0)(
        molecules[0]['positions'])
