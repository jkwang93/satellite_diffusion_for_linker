...
from zinc.data import ProcessedDataset, PreprocessQM9
...
try:
    ...
    use_rdkit = False
import zinc.dataset as dataset
import torch
import matplotlib
...
def main_analyze_qm9(remove_h: bool, dataset_name='zinc', n_atoms=None):
    class DataLoaderConfig(object):
        def __init__(self):
            ...
            self.dataset = dataset_name  # could be zinc, qm9_first_half or qm9_second_half
...
def check_stability(positions, atom_type, dataset_info, debug=False):
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            ...
            elif dataset_info['name'] == 'geom' or dataset_info['name'] == 'zinc':
...
def process_loader_(dataloader):
    """ Mask atoms, return positions and atom types"""
    out = []
    count = 0
    for data in dataloader:
        for i in range(data['positions'].size(0)):
            positions = data['positions'][i].view(-1, 3)
            one_hot = data['one_hot'][i].view(-1, 14).type(torch.float32)
            mask = data['atom_mask'][i].flatten()
            smiles_out = data['smiles_out'][i]
            graph_out = data['graph_out'][i]
            positions, one_hot = positions[mask], one_hot[mask]
            atom_type = torch.argmax(one_hot, dim=1)
            out.append((positions, atom_type,smiles_out,graph_out))
...
def main_check_stability(remove_h: bool, batch_size=32):
    ...
    dataset_info = datasets_config.zinc
    from torch.utils.data import DataLoader

    path = '../data/molecules_zinc_test_final.json'  # train/valid/test文件格式一致
    with open(path, 'r') as f:
        train_data = json.load(f)
    datasets = {'train': train_data}
    preprocess = PreprocessQM9(load_charges=True)

    datasets = {split: ProcessedDataset(data, shuffle=False) for split, data in datasets.items()}
    ...
    train_loader = process_loader_(dataloaders['train'])
    test_loader = process_loader_(dataloaders['train'])
    if use_rdkit:
        print('For test')
...
if __name__ == '__main__':
    # main_analyze_qm9(remove_h=False, dataset_name='zinc')
